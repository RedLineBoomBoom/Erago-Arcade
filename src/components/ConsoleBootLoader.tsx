...
export const ConsoleBootLoader: React.FC<ConsoleBootLoaderProps> = ({ onComplete, onSkip }) => {
  ...
  // Attempt audio unlock on user interaction
  const handleUserInteraction = useCallback(() => {
    setHasInteracted(true);
    sound.resumeAudio();
  }, []);

  // Immediate Boot Sequence execution on mount
  useEffect(() => {
    // 0ms: Attempt audio resume non-blockingly
    sound.resumeAudio();

    // Phase 1: CRT Warmup (0 - 650ms)
    sound.playRelayClick();
    sound.playCrtBuzz();
    setProgressPercent(15);

    // Phase 2: BIOS Diagnostics (650ms - 2500ms)
    addTimeout(() => {
      setPhase('bios');
      setVisibleBiosLines([]);
      ...
      BIOS_LOGS.forEach((item, idx) => {
        addTimeout(() => {
          ...
          // Smooth progress update up to 75%
    ...
    // Phase 3: Logo Reveal & Chime (2700ms - 4300ms)
    addTimeout(() => {
      setPhase('logo');
    ...
    // Phase 4: Ready Phase (4400ms - 5800ms)
    addTimeout(() => {
      setPhase('ready');
    ...
    // Phase 5: Automatic exit to main app (5900ms)
    addTimeout(() => {
      handleExit();
    }, 5900);
  ...
  }, [handleExit, handleUserInteraction, phase]);
        ...
        <div className="flex items-center justify-between text-[8px] sm:text-[9px] text-zinc-500 font-mono">
          <div className="flex items-center gap-2">
            ...
            {!hasInteracted && !isMuted && (
              <span className="hidden sm:inline text-amber-400/90 font-bold animate-pulse">
                • {t('boot_click_sound')}
              </span>
            )}
          </div>
          <span className="hidden sm:inline">60 FPS // PROCEDURAL AUDIO</span>
        </div>
      </div>
    </div>
  );
